...
TEST(CoalDArcCompensationUnit, FactoryParsesConfig)  // NOLINT
{
  ...
  mgr->setActiveCollisionObjects(std::vector<tesseract::common::LinkId>{ "link_a" });
...
TEST(CoalDArcCompensationUnit, MissedCollisionCaught)  // NOLINT
{
  ...
  auto run_test = [&](bool compensation) -> bool {
    ...
    checker->setActiveCollisionObjects(std::vector<tesseract::common::LinkId>{ "link_a", "obstacle" });
