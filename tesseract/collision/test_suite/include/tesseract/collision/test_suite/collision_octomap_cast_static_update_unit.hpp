...
{
  checker.setActiveCollisionObjects(std::vector<tesseract::common::LinkId>{ "active_cylinder" });
...
inline void runActiveOctreeDemotionClearsSweepState(ContinuousContactManager& checker)
{
  ...
  checker.setActiveCollisionObjects(std::vector<tesseract::common::LinkId>{ "moving_octree" });
  ...
  checker.setActiveCollisionObjects(std::vector<tesseract::common::LinkId>{ "probe_cylinder" });
