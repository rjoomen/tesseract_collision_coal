...
inline void addMultiShapeCollisionObjects(ContinuousContactManager& checker)
{
  ...
  ////////////////////////////
  CollisionShapePtr static_box = std::make_shared<tesseract_geometry::Box>(0.5, 0.5, 0.5);
  ...
  ////////////////////////////////////////////////////////////////
  CollisionShapePtr arm_box_a = std::make_shared<tesseract_geometry::Box>(0.2, 0.2, 0.2);
  CollisionShapePtr arm_box_b = std::make_shared<tesseract_geometry::Box>(0.2, 0.2, 0.2);
  ...
  tesseract_common::VectorIsometry3d arm_poses;
...
inline void runTestMultiShapeCast(ContinuousContactManager& checker)
{
  ...
  EXPECT_TRUE(cr.cc_transform[arm_idx].isApprox(pose2, 0.01))
      << "arm_link cc_transform should match end pose (90 deg rotation around Z). "
      << "If it doesn't, the cc_transform recovery may not account for local shape offsets.\n"
      << "Expected:\n"
      << pose2.matrix() << "\nGot:\n"
      << cr.cc_transform[arm_idx].matrix();

  // transform should be the link transform at t=0 (pose1)
  EXPECT_TRUE(cr.transform[arm_idx].isApprox(pose1, 0.01))
      << "arm_link transform should match start pose (identity).\n"
      << "Expected:\n"
      << pose1.matrix() << "\nGot:\n"
...
inline void runTestMultiShapeDiscrete(DiscreteContactManager& checker)
{
  ...
  ASSERT_FALSE(result_vector.empty())
      << "Multi-shape discrete: expected collision between sub-shape A (at (0,1,0) after 90 deg "
      << "rotation) and static obstacle at (0,1,0).";
}

}  // namespace detail

inline void runTestContinuous(ContinuousContactManager& checker)
{
  detail::runTestMultiShapeCast(checker);
}

inline void runTestDiscrete(DiscreteContactManager& checker)
{
  detail::runTestMultiShapeDiscrete(checker);
}
